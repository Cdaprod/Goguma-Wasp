...
const ReportPage = () => {
  ...
  const [updatedTitle, setUpdatedTitle] = useState(report?.title || '');
  const [updatedContent, setUpdatedContent] = useState(report?.content || '');
  const [updatedImage, setUpdatedImage] = useState(report?.image || '');
  const [updatedPdf, setUpdatedPdf] = useState(report?.pdf || '');
  ...
  return (
    <div className='p-4'>
      ...
      <input
        ...
        placeholder='PDF URL'
        value={updatedPdf}
        onChange={(e) => setUpdatedPdf(e.target.value)}
        className='px-1 py-2 border rounded text-lg mb-2'
      />
      <button
